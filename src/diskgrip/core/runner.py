...
Vendored from netgrip (kept deliberately standalone, see AGENTS.md). Only the
user-facing branding strings differ from the upstream copy.
...
# diskgrip runs on Windows purely as an SSH client: there is no local `ip`, no
...
# diskgrip is launched as a GUI app on Windows (no-op elsewhere).
...
def _askpass_helper() -> str:
    ...
    whatever is in ``$DISKGRIP_ASKPASS`` — the password lives only in the ssh/sudo
    ...
    directory = tempfile.mkdtemp(prefix="diskgrip-")
    if IS_WINDOWS:
...
class LocalRunner(Runner):
    ...
    def run_privileged(self, commands: list[list[str]]) -> str:
        ...
        if IS_WINDOWS:
            raise CommandError(
                ...
                1,
                "diskgrip can't manage the local machine on Windows; connect to a "
    ...
    def _pick_escalation(self) -> list[str]:
        ...
        raise CommandError(
            ...
            1,
            "Cannot gain root: run diskgrip as root, configure passwordless sudo "
...
class DemoRunner(Runner):
    ...
    def run_privileged(self, commands: list[list[str]]) -> str:
        raise CommandError(
            batch_script(commands),
            1,
            "Demo mode: this is exactly what diskgrip would have run on a real host, "
